 ...
 * @param initialDelay is a delay before starting the gradual text appear, given in ms.
 * @param wordSpeed is speed of words given in ms.
 * The interval will only tick as fast as the wordSpeed so initialDelay will will be rounded to minimum of wordSpeed
 * @param setGradualTextDone function which is used to tell the owner of this component that all the text given has been displayed.
 * @param children the text to be revealed is given as children.
 */
const GradualText = ({
  initialDelay = 1000,
  wordSpeed = 100,
  ...
  const wordArray: string[] | undefined =
    typeof children === 'string' ? children?.split(' ') : undefined;
  ...
  return <p style={{ fontSize: '18px', color: 'white', width: "95%" }}>{words}</p>;
};
